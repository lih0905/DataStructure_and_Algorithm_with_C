...
unsigned int GetNextToken(char *Expression, char *Token, int *TYPE)
{
    ...
    for (i = 0; 0 != Expression[i]; i++)
    {
        ...
        // 숫자가 처음부터 계속해서 나오면 그 숫자를 모두 추출하여 Token에 저장하고
        // TYPE은 OPERAND 로 저장
        // 반면 첫 문자가 숫자가 아니면 그 연산자를 TYPE, Token에 저장
...
double Calculate(char *PostfixExpression)
{
    ...
    while (Read < Length)
    {
        ...
        else
        {
            ...
            Operator2 = atof(OperatorNode->Data); // string -> float
            ...
            gcvt(TempResult, 10, ResultString); // float -> string
